...
public class ConfigUtil {
    ...
    static {
        TIME_UNIT_SLOTS = new long[]{TimeUnit.DAYS.toMillis(1L),
        ...
        for (int i = 0; i < TIME_UNIT_SLOTS.length; ++i) {
            ...
            if (r >= 1.0) {
                if (millis % TIME_UNIT_SLOTS[i] == 0L) {
                    return String.format("%d %s", (long) r, TIME_UNITS_CN[i]);
                }

                return String.format("%.1f %s", r, TIME_UNITS_CN[i]);
            }
        }

        return String.format("%d %s", millis, TIME_UNITS_CN[TIME_UNITS_CN.length - 1]);
    }
